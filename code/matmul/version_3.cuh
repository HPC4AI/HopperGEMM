...
__global__ void kernel(int M, int N, int K, 
    ...
    constexpr int wgmma_m = 64, wgmma_n = 128, wgmma_k = 16;
            ...
            for(int im = 0; im < tile_m / wgmma_m; im ++) {
                ...
                for(int ik = 0; ik < tile_k / wgmma_k; ik ++) {
                    wgmma<tile_k>(frag_c+im*2,
                            smem_a + (ok%num_stage)*tile_m*tile_k +im*wgmma_m*tile_k+ik*wgmma_k,
        ...
        for(int im = 0; im < tile_m / wgmma_m; im++) {
            ...
            for(int rt = 0; rt < 2; rt ++) {
                ...
                for(int ct = 0; ct < 16; ct ++) {
                    int i = im * wgmma_m + warp * 16 + rt * 8 + row_offset;
...
void runKernel3(int M, int N, int K, bf16 *A, bf16 *B, float *C) {
    constexpr int tile_m = 128;
    constexpr int tile_n = 128;
    constexpr int tile_k = 64;
    constexpr int num_stage = 5;

    constexpr int smem_size = num_stage * (tile_m * tile_k + tile_n * tile_k) * sizeof(bf16) / 1024;
    printf("smem_size=%d\n", smem_size);
